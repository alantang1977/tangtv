...
这个 APK 的核心价值与当前项目的播放链路高度匹配：它直接读取 ExoPlayer 解码后的 PCM，不依赖麦克风，也不需要第二套识别模型；当前项目已经具备 PCM tap、单声道处理、16 kHz 重采样、Sherpa-ONNX 和相对 seek 能力。因此最小融合路径是把现有 PCM tap 改成扇出，同时给语音识别和频谱指纹匹配器供数。
...
这与当前项目的 `RealtimeSubtitleAudioProcessor` 属于同一层级能力。融合时不应再安装一个独立的 `AudioProcessor`，而应把现有处理器提升为共享 PCM tap，避免重复复制、重复 downmix 和重复重采样。
...
- `AdSignalCoordinator` 负责去重、优先级和同一时间窗口的合并。

### 3.2 推荐的模块边界

| 模块 | 责任 | 不负责 |
|---|---|---|
| `PlaybackAudioTap` | 从 ExoPlayer PCM 得到单声道样本，并向多个消费者扇出 | seek、广告判断、持久化规则 |
| `AudioFingerprintMatcher` | 复用 `spectral-sequence-v2`，维护滚动 hash 和匹配状态 | UI、网络加载、UserAdRule |
| `FingerprintRuleStore` | 管理签名后的指纹包、版本、缓存和回滚 | 运行时跳过 |
| `FingerprintAdSignal` | 将 START/FULL 映射为统一 `AdSignal`，附带 ruleId、置信度、预测结束位置 | 直接调用播放器 |
| `VoiceAdSignal` | 接收现有 Sherpa-ONNX 识别结果并生成语音信号 | 写入指纹库 |
| `AdSignalCoordinator` | 处理信号优先级、同窗去重和冷却 | 解析 HLS |
| `AdSkipController` | 校验点播/seek/duration、展示确认、执行和撤销跳过 | 规则采集和上传 |
...
客户端需要校验 HTTPS、签名、算法版本、包大小、规则数、单规则帧数和过期时间；写入采用临时文件加原子替换，并保留上一份可回滚快照。规则包下载地址不能由普通用户输入，避免把 `RuleLoader` 变成任意 HTTP 请求入口。
...
- 从 SDK 核心源码或明确授权的等价实现中抽出 JVM 可测试模块；
- 接入当前 `PlaybackAudioTap` 的 PCM16/float 两种输入；
- 建立 10 至 20 条真实广告的黄金向量和跨编码样本；
- 验证 Android 与 H5 的 hash 序列完全一致；
- 不打开自动跳过，只输出诊断日志和 `START/FULL` 事件。

### Phase 1：运行时观察模式

- 在现有 PCM tap 上扇出 Sherpa 和指纹 matcher；
- 增加 `FingerprintRuleStore`、feature flag 和独立缓存；
- 记录 ruleId、匹配帧数、置信度、预测结束位置、处理耗时、队列峰值和 reset；
- 与语音信号去重，但不影响用户播放。
...
- 仅对签名且质量为 GOOD 的规则启用；
- ExoPlayer 点播默认确认后跳过；
- 复用现有 `AdSkipController` 的 seek 钳制、不可 seek 降级和撤销；
- 用 FULL_MATCHED 结果回填质量统计，发现错配自动降级规则。
...
2. 把该 SDK 的频谱序列 matcher 接成第三个运行时信号通道，已知广告优先使用指纹，未知广告继续由语音兜底；
3. 指纹规则独立于 `UserAdRule`，先做签名、版本、质量和回滚，再做线上自动下发；
4. 首版只做 ExoPlayer 点播、确认后跳过和观察指标，稳定后再开放自动跳过；
5. 在指纹库扩大前优先解决 H5/Android 黄金向量、规则治理和候选索引，否则“接入方便”会很快被覆盖率、误命中和性能问题抵消；
6. 在确认 SDK 源码和规则数据许可证前，不直接复制 APK 代码或将其作为项目依赖发布。

该方案值得进入现有智能去广路线，但下一步应先完成 Phase 0 的兼容性 Spike；只有黄金向量、规则包安全和低端设备性能通过验证，才适合把指纹信号写入主去广控制器。
